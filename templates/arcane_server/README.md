...
**Access pattern:**
```dart
final user = await ArcaneServer.svcUser.getUser(userId);
```
...
```dart
ArcaneServer.svcMedia.initialize("FIREBASE_PROJECT_ID.appspot.com");
```
...
```dart
final result = await ArcaneServer.svcMy.processData(data);
```
...
| arcane_models | Shared data models |

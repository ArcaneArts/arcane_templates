...
class UserAPI implements Routing {
  ...
  Future<Response> _getUserInfo(Request request, String userId) async {
    try {
      ...
      final user = await {{class_name.pascalCase()}}Server.svcUser.getUser(userId);
  ...
  Future<Response> _updateUser(Request request, String userId) async {
    try {
      ...
      await {{class_name.pascalCase()}}Server.svcUser.updateUser(userId, data);
  ...
  Future<Response> _listUsers(Request request) async {
    try {
      ...
      final users = await {{class_name.pascalCase()}}Server.svcUser.listUsers(limit: limit);
